//! Migrations, embedded at compile time so the binary never reads a
//! schema file at run time. Each migration runs in one transaction and
//! advances the version row in the same transaction.
