//! The ranking blend. Single point of change for the calibration
//! constants; a re-tune is a recompile, never a migration.

/// Calibration, revised against measured data.
struct Blend {
...
/// Frecency saturation constant, about two weeks of daily visits. Exported
/// as the single source for any downstream calibration; the picker's
/// signal meter that used to consume it was removed, so it currently has
/// no consumer.
...
/// rendered rows below it (streaming stability), which is why
...
/// Total order over ranked candidates: rank
