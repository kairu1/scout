...
/// `doctor` never modifies, migrates or repairs. The normal index open
/// path creates directories, creates
...
/// Discovery refuses to follow a symlink, so a symlinked config is skipped in silence and the user
...
/// A closed allowlist, because this output exists to
...
/// This output exists to be pasted. Environment values and
