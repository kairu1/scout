...
| `{env.NAME}` | The value of `NAME` in the action-scope env (§3, `env` step). Undefined → abort per ADR-003 §5. The scope starts **empty** and accumulates only via `env` steps in the same action; it does **not** seed from SCOUT's parent environment (ADR-003 §129, and its 2026-08-14 revision — seeding it made a failed `env` step resolve to whatever the user had exported, the substitution the threat model refuses). `NAME` must match `[A-Za-z_][A-Za-z0-9_]*`, 1–64 chars; malformed references are a parse error. |
