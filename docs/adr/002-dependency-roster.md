...
- 2026-07-06 — three Phase 4 packaging ratifications, commander-authorised (shadow-review pass, HANDOFF 2026-07-06). (1) **Transitive ceiling metric.** The <120 ceiling is measured against the *shipped* target graph (`cargo tree --prefix none`, currently 104), not `cargo tree --target all` (124) — the four-crate overage there is entirely Windows-only crates under `crossterm`, for a platform this ADR defers (§Cross-compile story, §Dirs resolution). CI enforces the shipped-target reading. (2) **Licence allow-list gains `Unicode-3.0` and `MPL-2.0`.** `Unicode-3.0`: modern `unicode-ident` relicensed to it (same Unicode terms). `MPL-2.0`: the actual licence of `nucleo-matcher` (slot 6) — the original ADR list was authored without checking the roster's own licences and would have rejected a signed roster crate; MPL-2.0 is file-level weak copyleft and imposes nothing on scout's MIT source when linking an unmodified build. The `cargo deny` tripwire caught both on first real run. (3) **`cargo deny` duplicate policy** is `multiple-versions = "deny"` with an explicit skip list (`hashbrown`, `itertools` — and `windows-sys`, all irreducible transitive duplicates — `windows-sys` is Windows-only, a platform this ADR defers), which realises this ADR's "fails on duplicate versions beyond a small allow-list" concretely. `deny.toml` in tree is the source. Status remains Accepted.
- 2026-08-14 — roster extended by one slot via **ADR-005 (Display Width)**, commander-directed. `unicode-width` pinned to `0.1` joins as a direct dependency. It was already in the shipped graph twice under `ratatui` (directly and through `unicode-truncate`), so the transitive ceiling is unchanged at 105 of 120 and no supply-chain cost is incurred — the crate was ruled out originally by this ADR's closed-set discipline, not by a considered refusal. The `0.1` requirement is imposed by our own `multiple-versions = "deny"` policy and moves only when `ratatui` moves. See ADR-005 for the layout contract. Status remains Accepted.
- 2026-08-14 — **dependency refresh (v2 objective 2), commander-directed.** Roster versions advanced: `rusqlite` 0.31 → 0.40, `ratatui` 0.26 → 0.30, `crossterm` 0.27 → 0.29, `thiserror` 1 → 2, `unicode-width` 0.1 → 0.2 (ADR-005), plus in-range moves for `ignore`, `clap`, `serde` and their transitives. All suites, clippy, `cargo deny`, `cargo audit` and the 100k perf gate green; the picker verified rendering unchanged by PTY capture, not by compilation alone. Four consequences worth recording.

  **(1) `thiserror` 2.x — this revision is the successor decision slot 11 called for.** Slot 11 pinned `thiserror` to `1` and deferred any 2.x bump to a successor ADR. The graph made the decision concrete rather than theoretical: `ratatui` 0.30 already carries `thiserror` 2, so remaining on 1 produced exactly the duplicate our own `multiple-versions = "deny"` policy rejects. The choice was not "1 or 2" but "2, or a permanent skip entry hiding a real duplicate". Slot 11 now reads `2`. `toml` 1.x remains deferred — nothing forces it and the 0.8 API is in use.

  **(2) The transitive ceiling was breached and answered by trimming, not by raising.** `ratatui` 0.30 split one crate into four (`ratatui`, `-core`, `-crossterm`, `-widgets`) and its default features (`all-widgets`, `macros`, `underline-color`, `layout-cache`) took the count to **127** against a limit of 120. Scout uses none of them: no calendar widget, no macro DSL, no underline colours, and `layout-cache` — which costs `critical-section` — buys nothing for three `Layout` splits per frame. With `default-features = false, features = ["crossterm_0_29"]` the count is **119**. A ceiling breach is first a question about what you are pulling in, and only second a question about the number.

  **(3) The skip list was reviewed, not merely appended to.** `syn` added: 2.x under ratatui's `derive_more`/`strum` macros, 3.x under `clap_derive`; both are proc-macro *build* dependencies that never reach the shipped artifact, so holding `clap` back to unify a compile-time graph would pay a real cost for a cosmetic one. `itertools` and `windows-sys` **removed** — both are now single-version, and a skip entry that no longer describes a real duplicate is not harmless: it is a silenced alarm for the next duplicate to appear under that name.

  **(4) The refresh cleared three standing advisories.** `cargo audit` previously carried `paste` (unmaintained) and two `lru` unsoundness advisories, all transitives beneath `ratatui` 0.26. `ratatui` 0.30 does not pull them. The audit is now clean with zero warnings, where before it was clean only by allowance.

  **Open, and a commander decision — the ceiling now has zero headroom.** 119 against a limit enforced as `< 120` means the next crate added anywhere in the graph fails CI. Two honest readings. Either the limit should rise, because the count metric is measuring the wrong thing: the `ratatui` split added three crate entries of the same code from the same maintainers, so the audit surface did not grow while the number did, and 22 of the 119 are `rust-lang`/BurntSushi/dtolnay/tokio crates that a supply-chain review would treat as one risk each at most. Or the limit should hold precisely *because* it is now binding, and every future addition should have to argue for itself. This ADR does not decide it; the number is recorded so the decision is taken deliberately rather than discovered by a red build.
- 2026-08-14 — **transitive ceiling raised from 120 to 150, commander-directed.** The refresh above left the count at 119 against a limit enforced as `< 120`: zero headroom, so the next crate added anywhere in the graph would fail CI regardless of merit. The count was also measuring the wrong thing. `ratatui` 0.30 split one library into four crates of the same code from the same maintainers — the audit surface did not grow, the number did — and 22 of the 119 entries are `rust-lang`/BurntSushi/dtolnay/tokio crates that a supply-chain review would treat as one risk each at most. 150 restores roughly 25% headroom, which is enough for the remaining v2 objectives (a grapheme-segmentation crate for editing, a serialisation crate if a JSON mode ships) without becoming a blank cheque: an async runtime or an embedded interpreter would still blow straight through it, which is the class of addition this gate exists to stop. The gate's purpose is unchanged — it is a tripwire against *unnoticed* growth, not a budget to be spent.
